...
import { detectIntent, extractNodes, generateOptions, generateSandboxCode } from '../generation';
...
export class LearnerSupportService {
  ...
  static async execute(
    ...
    // Phase 1: 意図検出
    const intentResult = await detectIntent(purposeAlpha);

    // Phase 2: ノード抽出（構造更新）
    // AIに現在の構造を渡し、更新された構造を受け取る
    const extractionResult = await extractNodes(purposeAlpha, intentResult, baseContextNodes as any[], logs as any[]);

    // Phase 3: 選択肢生成
    // 更新されたノード構造に基づいて選択肢を生成
    const optionsResult = await generateOptions(purposeAlpha, extractionResult.nodes, state);

    // AIの結果を正として採用
    // ただし、reliabilityの補完などは行う
    const finalNodes = extractionResult.nodes.map((n: any) => ({
      ...n,
      id: n.id,
      label: n.label,
    ...
    // 3. RESOLVED時のみsandbox生成
    if (state === 'RESOLVED') {
      const sandboxCode = await generateSandboxCode(
        purposeAlpha,
